...
class SharedPrefs {
  ...
  Future<bool> setLoginData(UserDetailsModel loginApi) async {
  ...
  Future<UserDetailsModel?> getLoginData() async {
    final sharedPrefs = GetIt.instance<SharedPrefs>();
    final prefs = sharedPrefs.prefs;
    String? res = prefs.getString("loginData");
    if (res != null) {
      Map<String, dynamic> api = json.decode(res);
      UserDetailsModel loginApiModel = UserDetailsModel.fromJson(api);
